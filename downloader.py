...
def parse_args():
    ...
    parser.add_argument('--region',type=str,help='Server region, the program will read resdata/{$REGION}_resdata.json to find assets')
    parser.add_argument('--downloadres',type=int,help='Whether to download resdata.zip, 1 for true and 0 for false',metavar='0/1')
    parser.add_argument('--url',type=str,help='URL for downloading resdata.zip, will override download_resdata if given')
    parser.add_argument('--abname',action='store_const',const=True,help='Save assets using assetBundleName instead of resname, useful for unpackers')
    group=parser.add_argument_group('Assets',description='Select assets you want to download (base/add/passivity), all settings in config.json5 will be overrided if any is passed')
    group.add_argument('-b',action='store_true',help='Download base assets (required to open the game)')
    group.add_argument('-a',action='store_true',help='Download additional assets (required for full gameplay experience)')
    group.add_argument('-p',action='store_true',help='Download passivity assets (optional assets: hd picture, voice, etc.)')
    ...
    if args.b or args.a or args.p:
        config['download_base_assets'],config['download_add_assets'],config['download_passivity_assets']=args.b,args.a,args.p
